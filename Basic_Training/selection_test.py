...
3등은 0번 학생이고 전국 대회에 참석 불가능 하므로 넘어갑니다. 4등과 5등은 각각 4번, 3번 학생이고 두 학생 모두 전국 대회에 참석 가능하므로 각각 2순위, 
3순위로 선발됩니다. 3명을 선발하였으므로 6등과 7등은 확인하지 않아도 됩니다. 따라서 10000 * 2 + 100 * 4 + 3 = 20403을 return 합니다.
    ...
    while len(n_rank) < 3:
        for i in range(1, len(rank)+1):
            if attendance[rank.index(i)] == True:
                n_rank.append(rank.index(i))
    a, b, c = n_rank[0], n_rank[1], n_rank[2]
